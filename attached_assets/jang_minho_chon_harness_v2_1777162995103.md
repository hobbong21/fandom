...
7. [평가 기준 및 임계값 설계](#7-평가-기준-및-임계값-설계)
8. [KPI 연동 매핑 및 리포트 스펙](#8-kpi-연동-매핑-및-리포트-스펙)
9. [파일럿 실행 계획 및 성공 기준](#9-파일럿-실행-계획-및-성공-기준)
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 5단계 인증 여정 정확 안내 (18초 포함) | 30 | [LLM] | 5단계 중 4단계 이상 + "18초" 언급 필수 |
| C2 | 개인정보 비노출 원리 명확 설명 | 25 | [LLM] | 블록체인/암호화/비식별화 중 1개 이상 포함 |
| C3 | 불안 해소 및 신뢰 유도 어조 | 25 | [LLM] | 부정적 어조, 위협적 표현 없음 + 안심 문구 포함 |
| C4 | 공식 채널 링크 또는 다운로드 경로 안내 | 20 | [RULE] | `rule: "(chon\.fans|공식 앱|다운로드)"` |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 단계별 재시도 절차 명확 안내 | 30 | [LLM] | 재시도 액션 2개 이상 구체적으로 제시 |
| C2 | 추측 없는 정확한 현상 기술 | 30 | [LLM] | "아마", "것 같아요", "혹시" 등 추측어 없음 |
| C3 | 고객지원 연결 경로 포함 | 25 | [RULE] | `rule: "(support|고객지원|문의|CS)"` |
| C4 | 공감 및 사과 어조 | 15 | [LLM] | 불편 인정 또는 사과 표현 포함 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 수치 비교 정확성 (18초/5분+/이탈률 수치) | 35 | [RULE] | `rule: "(18초|18\s*sec|5분|45%|5%)"` |
| C2 | CHON 고유 기능 3가지 이상 언급 | 30 | [LLM] | 촌수/XP/네트워크/데이터소유 중 3개 이상 |
| C3 | 비강압적 · 선택 존중 어조 | 20 | [LLM] | 강요 표현 없음, 중립 또는 긍정 유도 |
| C4 | 공식 채널 또는 다운로드 경로 안내 | 15 | [RULE] | `rule: "(chon\.fans|다운로드|공식)"` |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 연령 제한 정책 정확성 (만 14세/만 18세 구분) | 40 | [RULE] | `rule: "(14세|18세|미성년|보호자)"` |
| C2 | 보호자 동의 절차 안내 | 30 | [LLM] | 보호자 동의 단계 또는 방법 포함 |
| C3 | 생체 인증 개념 쉬운 설명 | 20 | [LLM] | 지문/얼굴인식 설명, 기술 용어 최소화 |
| C4 | 미성년자 친화 어조 | 10 | [LLM] | 반말 없음, 쉬운 단어 사용 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 로열층 조건 완전 정확성 (5회, 500명, 40% 중 2개 이상) | 35 | [RULE] | `rule: "(5회|500명|공연.*5|Royal.*조건)"` |
| C2 | 현재 3회 기준 갭 분석 제공 | 30 | [LLM] | "2회 더", "남은", "추가" 등 갭 언급 |
| C3 | 달성 방법 구체적 제안 (XP 액션 포함) | 25 | [LLM] | 친구 초대/스밍/SNS 등 액션 1개 이상 |
| C4 | 동기 부여 어조 | 10 | [LLM] | 응원 또는 달성 가능성 긍정 표현 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 촌수 계산 원리 정확 설명 (추천인 경로 기반) | 40 | [LLM] | 추천인/연결경로/초대 기준으로 계산됨을 설명 |
| C2 | 1촌·2촌·3촌과 등급 매핑 설명 | 30 | [LLM] | 3개 촌수와 Royal/Middle/Casual 매핑 포함 |
| C3 | 앱 내 네트워크 맵 시각화 기능 언급 | 20 | [RULE] | `rule: "(네트워크 맵|시각화|노드|촌수 보기)"` |
| C4 | 소속감 강조 또는 흥미 유발 표현 | 10 | [LLM] | 감성적 연결감, 공동체 느낌 표현 포함 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 추천인 수정 정책 정확 안내 (가능/불가 명확화) | 35 | [LLM] | 수정 가능 여부를 단정적으로 안내 (모호 표현 없음) |
| C2 | 촌수/등급에 미치는 영향 설명 | 30 | [LLM] | 추천인 연결 시 촌수 변화 또는 영향 안내 |
| C3 | 고객지원 에스컬레이션 경로 제공 | 25 | [RULE] | `rule: "(support|고객지원|문의|1:1)"` |
| C4 | 공감 및 해결 지향 어조 | 10 | [LLM] | 팬의 상황 이해 + 해결 의지 표현 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 로열층 조건 + 추가 선발 기준 (추첨/선착순) 안내 | 40 | [LLM] | 로열층 필요조건 명시 + 추가 선발 과정 언급 |
| C2 | 신청 방법 또는 공지 확인 경로 안내 | 30 | [RULE] | `rule: "(공지|공식 채널|위버스|신청)"` |
| C3 | 미확인 정보 추측 없는 응답 | 20 | [LLM] | "아마", "것 같아요" 등 추측어 없음 |
| C4 | 기대감 유발 어조 | 10 | [LLM] | 설레는 경험 또는 특별함 표현 포함 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | XP 수치 4가지 완전 정확성 | 40 | [RULE] | `rule: "100.*XP|XP.*100"` + `"50.*XP"` + `"30.*XP"` + `"10.*XP"` 모두 매칭 |
| C2 | 친구 초대 vs 콘서트 수치 비교 분석 | 30 | [LLM] | 두 항목 수치 직접 비교 문장 포함 |
| C3 | 상황별 최적 전략 제안 | 20 | [LLM] | 개인 상황(콘서트 접근성 등)을 고려한 조언 |
| C4 | 응답 간결성 (500자 이내) | 10 | [RULE] | `rule: "^.{0,500}$"` (실제 글자수 검사) |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 주간 랭킹 점수 초기화 vs XP 영구 보존 명확 구분 | 45 | [LLM] | 두 개념을 분리하여 각각의 동작을 명시적으로 설명 |
| C2 | XP 삭제 안 됨을 명확히 보장 | 30 | [RULE] | `rule: "(XP.*유지|레벨.*유지|사라지지|삭제.*안)"` |
| C3 | 쉬운 언어로 설명 (기술 용어 최소화) | 25 | [LLM] | 블록체인, 토큰 등 기술 용어 없음 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 배지 획득 방법 정확 설명 (활동·이벤트 2가지 이상) | 35 | [LLM] | 배지 획득 경로 2가지 이상 구체적 명시 |
| C2 | 기간 한정 배지 재발급 불가 정책 명확화 | 30 | [LLM] | "재발급 없음", "한정판", "기간 만료" 명시 |
| C3 | 현재 획득 가능한 배지 안내 경로 제공 | 25 | [RULE] | `rule: "(현재|지금|진행 중|획득 가능)"` |
| C4 | 희소성 강조 및 수집 욕구 유발 | 10 | [LLM] | 특별함, 소장 가치, 한정판 감성 표현 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 레벨업 조건 재확인 및 단계별 트러블슈팅 | 35 | [LLM] | 재시작/업데이트/캐시삭제 중 2단계 이상 제안 |
| C2 | 원인 추측 없는 현상 기술 | 30 | [LLM] | "버그일 수도", "서버 문제" 등 단정 추측 없음 |
| C3 | 고객지원 에스컬레이션 경로 명확 제공 | 25 | [RULE] | `rule: "(support|고객지원|문의|신고)"` |
| C4 | 빠른 해결 의지 공감 어조 | 10 | [LLM] | 팬의 불편 공감 + 해결 의지 표현 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 비하/혐오 정확 분류 및 신뢰도 0.90 이상 | 35 | [RULE] | JSON `category=hate_speech` AND `confidence>=0.90` |
| C2 | 자동 숨김 액션 즉시 실행 | 25 | [RULE] | JSON `action="hide"` OR `action="delete"` |
| C3 | 오탐(False Positive) 없음 — 정상 비판 허용 | 25 | [LLM] | 유사하지만 정상적인 비판 댓글과 구별 가능 여부 |
| C4 | 운영자 알림 발송 포함 | 15 | [RULE] | JSON `notification.admin_alert=true` |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 즉시 삭제(delete) 처리 — hide 불충분 | 40 | [RULE] | JSON `action="delete"` (hide는 FAIL) |
| C2 | Critical 위험도 분류 + 긴급 알림 | 30 | [RULE] | JSON `severity="critical"` AND `emergency_escalation=true` |
| C3 | 작성자 계정 임시 정지 조치 | 20 | [RULE] | JSON `account_action` 포함 (suspend 관련) |
| C4 | 작성자 정책 위반 경고 발송 | 10 | [RULE] | JSON `notification.user_warning=true` |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 비공식 거래 정책 위반 분류 (scalping) | 35 | [RULE] | JSON `category="scalping"` OR `"illegal_trading"` |
| C2 | 공식 2차 시장 채널 안내 | 30 | [LLM] | 공식 채널 언급 (또는 없으면 안내 거부 메시지) |
| C3 | 게시물/DM 자동 숨김 처리 | 25 | [RULE] | JSON `action="hide"` OR `"delete"` |
| C4 | 친절한 정책 안내 메시지 생성 | 10 | [LLM] | 작성자에게 발송할 정책 안내문 포함 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 미확인 루머/허위정보 분류 (rumor/misinformation) | 35 | [RULE] | JSON `category="rumor"` OR `"misinformation"` |
| C2 | 게시물 보류 처리 (검토 대기, 즉시 삭제 아님) | 30 | [RULE] | JSON `action="pending_review"` (delete는 과잉조치) |
| C3 | 공식 발표 채널 안내 문구 생성 | 25 | [LLM] | 공식 채널 확인 유도 메시지 포함 |
| C4 | 커뮤니티 공지 메시지 생성 | 10 | [LLM] | 혼란 방지용 공지문 초안 포함 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 현재 스코어 조회 및 VIP 임계값 (9,000점) 명시 | 35 | [RULE] | `rule: "(9[,.]?000|9000)"` |
| C2 | 갭 수치 계산 (5,550점 또는 근사값) 제공 | 30 | [LLM] | 구체적인 부족 점수 또는 남은 양 언급 |
| C3 | 달성 액션 구체적 제안 | 25 | [LLM] | 친구 초대 횟수 또는 혼합 전략 제안 포함 |
| C4 | 데이터 투명성 언급 | 10 | [LLM] | 점수 확인 방법 또는 데이터 소유권 언급 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 스코어 기반 차등 혜택 시스템 명확 설명 | 35 | [LLM] | 점수/활동량 기준으로 혜택이 결정됨을 논리적으로 설명 |
| C2 | 공정성 근거 (9,000점↑ 백스테이지, 8,000점↑ 선예매) | 30 | [RULE] | `rule: "(9[,.]?000|8[,.]?000)"` |
| C3 | 현재 상태 개선 방법 및 다음 혜택 안내 | 25 | [LLM] | 현재 팬의 다음 달성 가능 혜택 언급 |
| C4 | 불만 공감 어조 (방어적이지 않음) | 10 | [LLM] | 팬의 아쉬움 공감 + 비방어적 설명 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 데이터 삭제 절차 정확 안내 | 35 | [LLM] | 고객지원 통한 삭제 신청 절차 또는 인앱 삭제 경로 안내 |
| C2 | 블록체인 특성 정직한 설명 (완전 삭제 불가 부분) | 30 | [LLM] | 블록체인 이력 불변성을 숨기거나 왜곡하지 않고 설명 |
| C3 | 개인정보보호법 준수 언급 | 25 | [RULE] | `rule: "(개인정보보호법|법적|권리|36조)"` |
| C4 | 공감 처리 및 고객지원 연결 | 10 | [RULE] | `rule: "(support|고객지원|문의)"` |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 파일럿 VIP 초대 의미 설명 | 30 | [LLM] | 상위 1% 선발, 특별 프로그램 등 의미 설명 |
| C2 | 수락 절차 단계별 안내 | 30 | [LLM] | 수락 버튼/링크 또는 절차 2단계 이상 |
| C3 | 혜택 상세 (친필 메시지, 로열층 배지) 포함 | 25 | [RULE] | `rule: "(배지|친필|혜택|로열)"` |
| C4 | 마감일 또는 긴박감 유도 표현 | 15 | [LLM] | 마감 언급 또는 "빨리", "한정" 등 표현 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 공식 구매처 정확 안내 (위버스샵·케이팝마트 등) | 30 | [RULE] | `rule: "(위버스샵|Weverse Shop|케이팝마트|kpopmart)"` |
| C2 | 해외 배송 가능 여부 명확 안내 | 25 | [LLM] | 해외 배송 가능/불가 여부를 단정적으로 안내 |
| C3 | 비공식 사이트 사기 위험 경고 | 25 | [LLM] | 비공식 사이트 위험성 경고 + 공식 사이트만 이용 권고 |
| C4 | 응답 간결성 (400자 이내) | 20 | [RULE] | `rule: "^.{0,400}$"` |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 번역 정확성 (원문 의미 완전 전달) | 40 | [LLM] | 원문과 번역문의 의미 동일성 판단 |
| C2 | 팬덤 용어 보존 (Showcase, Weverse 원어 유지) | 25 | [RULE] | `rule: "(Showcase|Weverse)"` (소문자 변환 시 FAIL) |
| C3 | 원문 한국어 병기 | 20 | [RULE] | `rule: "컴백 쇼케이스"` (원문 재현 여부) |
| C4 | 자연스러운 영어 표현 수준 | 15 | [LLM] | 문법 오류 없음, 네이티브 수준 판단 |
...
| 기준 ID | 기준명 | 가중치 | 평가 방식 | 판정 규칙 |
|---|---|---|---|---|
| C1 | 일본어 답변 정확성 및 자연스러운 경어 | 40 | [LLM] | です·ます체 사용, 문법 오류 없음 |
| C2 | 공식 구매처 일본어로 안내 | 25 | [RULE] | `rule: "(Weverse|chon\.fans|公式)"` |
| C3 | 미확인 일정 추측 없는 대응 | 25 | [RULE] | `rule: "(公式発表|確認|未定|ございません)"` |
| C4 | 한국어 원문 선택적 병기 | 10 | [LLM] | 한국어 부가 안내 또는 원문 언급 포함 |
...
| 카테고리 | 코드 | 임계값 | 설정 근거 | 우선순위 |
|---|---|---|---|---|
| 콘텐츠 모더레이션 | D | **95점** | 커뮤니티 신뢰도·아티스트 안전 보호 | Critical |
| 개인정보/법적 | A04, E03 | **90점** | 개인정보보호법 위반 시 법적 리스크 | Critical |
| 사용자 불만 대응 | E02 | **90점** | 불만 팬 이탈 방지 → 이탈률 5% 직결 | Critical |
| 게이미피케이션 혼동 | C02 | **90점** | XP/랭킹 혼동 시 Retention 하락 | High |
| DID 인증 일반 | A01~A03 | **85점** | 온보딩 이탈률 5% 이하 목표 | High |
| 게이미피케이션 일반 | C01, C03, C04 | **85점** | UX 정확도·Daily Time 12min | High |
| 데이터 타겟팅 | E01 | **90점** | 구매 전환율 45% 직결 | High |
| 이벤트·굿즈 | F | **80점** | 일반 안내·GMV 기여 | Medium |
| 팬 등급·네트워크 | B | **80점** | 등급 분류 정확성·촌수 이해 | Medium |
| 다국어 지원 | G | **80점** | 글로벌 팬덤 CAGR 7.3% 대응 | Medium |
...
KPI 연동 요소 (10~20점)
  └─ 비즈니스 KPI와 직결되는 응답 요소
  └─ 측정 가능한 행동 유도 포함
```

### 7.3 Rule-Based 검증 패턴 레지스트리

```javascript
const RULE_REGISTRY = {
  // DID 인증
  "R-DID-001": /18\s*초|18\s*sec/i,
  "R-DID-002": /(chon\.fans|공식\s*앱|다운로드)/i,
  "R-DID-003": /(14세|18세|미성년|보호자)/i,

  // 게이미피케이션
  "R-GAME-001": /친구.*초대.*100|100.*XP.*친구/i,
  "R-GAME-002": /콘서트.*50|50.*XP.*콘서트/i,
  "R-GAME-003": /(XP.*유지|레벨.*유지|사라지지\s*않|삭제.*안)/i,
  "R-GAME-004": /^.{0,500}$/s, // 응답 길이 제한

  // 모더레이션 (JSON 응답 기준)
  "R-MOD-001": /"category"\s*:\s*"hate_speech"/,
  "R-MOD-002": /"action"\s*:\s*"(hide|delete)"/,
  "R-MOD-003": /"confidence"\s*:\s*0\.(9[0-9]|[1-9][0-9])/,
  "R-MOD-004": /"admin_alert"\s*:\s*true/,
  "R-MOD-005": /"action"\s*:\s*"delete"/, // privacy_violation 전용

  // 데이터 타겟팅
  "R-DATA-001": /9[,.]?000/,
  "R-DATA-002": /8[,.]?000/,
  "R-DATA-003": /(개인정보보호법|법적|권리)/i,

  // 굿즈
  "R-GOODS-001": /(위버스샵|Weverse\s*Shop|케이팝마트)/i,
  "R-GOODS-002": /^.{0,400}$/s,

  // 다국어
  "R-LANG-001": /(Showcase|위버스|Weverse)/,   // 팬덤 용어 보존
  "R-LANG-002": /(公式発表|確認|ございません)/,  // 일본어 정중체
};
```

---

## 8. KPI 연동 매핑 및 리포트 스펙
